...
def main(args):
    ...
    base_a = SoftModularizedMLP(3, np.prod(state_shape), args.taskid_dim, 256, 3, 256).to(args.device)
    base_r = SoftModularizedMLP(3, np.prod(state_shape), args.taskid_dim, 256, 3, 256).to(args.device)
    base_c = SoftModularizedMLP(3, np.prod(state_shape), args.taskid_dim, 256, 3, 256).to(args.device)
