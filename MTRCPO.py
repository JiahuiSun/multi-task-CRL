...
class MTRCPO:
    ...
    def learn(self):
        ...
        for epoch in tqdm(range(self.n_epoch)):
            ...
            for repeat in range(self.repeat_per_collect):
                ...
                all_task_actor_loss, all_task_critic_loss = 0, 0
    ...
    def rollout(self, sub_task_list=[]):
        ...
        tstep = 0
        while True:
            actions, log_probs = self.get_action(obs, env_idx_param)
    ...
    def compute_gae(self, buffer):
        ...
        for task, data in buffer.items():
            with th.no_grad():
                vs, cost_vs, _ = self.evaluate(data['obs'], np.array(task))
                vs_next, cost_vs_next, _ = self.evaluate(data['obs_next'], np.array(task))
